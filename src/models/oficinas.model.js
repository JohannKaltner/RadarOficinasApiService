...
Oficinas.findByBairro = function (Bairro, result) {
    dbConn.query("Select * from oficinas where bairro = ? ", Bairro, function (err, res) {             
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Oficinas.getPagination = function(page, limit, result){
    ...
    const offset = (pagina - 1) * limit
     dbConn.query("Select * from oficinas limit "+ limit +" OFFSET "+ offset , function (err, res) {             
        if(err) {
            console.log("error: ", err);
            result(err, null);
        }
...
Oficinas.update = function(id, oficinas, result){
  dbConn.query("UPDATE oficinas SET nome=?,cnpj=?,rua=?,numero=?,bairro=?,cidade=?, estado=?, latitude=?, longitude=?, id_usuario =?, criadoEm=?, atualizadoEm=?,cep=?, ddd=?, telefone1=?, telefone2=?, WHERE id = ?", [oficinas.nome,oficinas.cnpj,oficinas.rua,oficinas.numero, oficinas.bairro, oficinas.cidade, oficinas.estado, oficinas.latitude, oficinas.longitude, oficinas.id_usuario, oficinas.criadoEm, oficinas.editadoEm, oficinas.cep, oficinas.ddd, oficinas.telefone1, oficinas.telefone2, id], function (err, res) {
        if(err) {
            console.log("error: ", err);
            result(null, err);
        }else{   
