...
Usuarios.findById = function (id, result) {
  dbConn.query(
    "Select * from usuariosradar where id = ? ",
    id,
    function (err, res) {
      if (err) {
        console.log("error: ", err);
...
Usuarios.update = function (id, usuario, result) {
  dbConn.query(
    "UPDATE usuariosradar SET primeiroNome=?,segundoNome=?,email=?,senha=?,adm=?,criadoEm=? WHERE id = ?",
    [
...
Usuarios.delete = function (id, result) {
  dbConn.query(
    "DELETE FROM usuariosradar WHERE id = ?",
    [id],
    function (err, res) {
      if (err) {
        console.log("error: ", err);
...
Usuarios.Login = function (email, senha, result) {
  dbConn.query(
    "SELECT id FROM usuariosradar WHERE email = ? senha = ?",
    [email, senha],
    function (err, res) {
      if (err) {
        console.log("error:", err);
